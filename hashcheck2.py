...
# ハッシュタグ生成
def generate_hashtags(text, include_fixed=True, top_n=15):
...
include_fixed = st.checkbox("固定ハッシュタグを含める", value=True)

if st.button("ハッシュタグを生成"):
    if user_input.strip() == "":
        st.warning("テキストを入力してください。")
    else:
        auto_tags, fixed_tags = generate_hashtags(user_input, include_fixed=include_fixed)
