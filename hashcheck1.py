...
def extract_org_hashtag(text):
    ...
    if match:
        ...
        clean_name = re.sub(r'[^\wぁ-んァ-ヶ一-龥ー]', '', org_name)
        return '#' + clean_name
    return None

# ハッシュタグ生成（固定ハッシュタグを含めるか選べるように）
def generate_hashtags(text, top_n=15, include_fixed_tags=True):
    keywords = extract_keywords(text, top_n)
    auto_tags = ['#' + re.sub(r'\s+', '', kw) for kw in keywords]

    # 団体名ハッシュタグを先頭に追加（あれば）
    org_tag = extract_org_hashtag(text)
    if org_tag and org_tag not in auto_tags:
        auto_tags = [org_tag] + auto_tags

    fixed_tags = []
    if include_fixed_tags:
...
# 固定ハッシュタグを付けるかどうか選択
use_fixed = st.checkbox("固定ハッシュタグを付ける", value=True)

if st.button("ハッシュタグを生成"):
    if user_input.strip() == "":
        st.warning("テキストを入力してください。")
    else:
        auto_tags, fixed_tags = generate_hashtags(user_input, include_fixed_tags=use_fixed)

        hashtags_output = " ".join(auto_tags + fixed_tags)
